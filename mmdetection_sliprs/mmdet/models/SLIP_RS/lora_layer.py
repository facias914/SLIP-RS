...
def scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=False):
    ...
    scale = D ** -0.5
    attn_scores = torch.matmul(q, k.transpose(-2, -1)) * scale  # [B, H, L_q, L_k]
...
def _none_or_dtype(x):
    ...
    if not torch.is_tensor(x):
        raise TypeError(f"Expected Tensor or None, but got {type(x)}")
    return x.dtype
...
def _canonical_mask(mask, mask_name, other_type, other_name, target_type, check_other=True):
    ...
    if not torch.is_tensor(mask):
        raise TypeError(f"{mask_name} must be a Tensor or None, but got {type(mask)}")

    # If mask is boolean, convert it to the same dtype as target_type
    if mask.dtype == torch.bool:
        mask = torch.where(
            mask,
            torch.tensor(0.0, dtype=target_type, device=mask.device),
            torch.tensor(float('-inf'), dtype=target_type, device=mask.device),
        )
    elif not mask.dtype.is_floating_point:
        raise TypeError(
            f"expected {mask_name} to be a boolean mask or floating point type, "
            f"but got dtype {mask.dtype}"
        )

    if check_other and other_type is not None:
        if mask.dtype != other_type:
            raise TypeError(
                f"{mask_name} and {other_name} must have the same dtype, "
                f"but got {mask.dtype} and {other_type}"
            )
