...
class ConvNeXt(nn.Module):
    ...
    def load_pretrain_weights(self, pretrain):
        url = convert_path_or_url_to_url(pretrain)
        state_dict = torch.hub.load_state_dict_from_url(url, map_location="cpu", check_hash=False)
        self.load_state_dict(state_dict, strict=True)

    def init_weights(self):
        self.apply(self._init_weights)
        print("Init weights successfully!!!")
        if self.pretrain is not None:
            self.load_pretrain_weights(self.pretrain)
            print("Load Dinov3 weights successfully!!!")
